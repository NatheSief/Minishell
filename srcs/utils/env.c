...
/*   Updated: 2024/12/23 20:00:50 by nsiefert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_env(char *str, t_shell *master)
{
	...
	while (tmp)
	{
		if (!ft_strncmp((const char *)tmp->content, str, ft_strlen(str)))
		{
			if (*((char*)tmp->content + ft_strlen(str) + 1) == '=')
				return (tmp->content + ft_strlen(str) + 2);
		}
		tmp = tmp->next;
	}
	return (NULL);
